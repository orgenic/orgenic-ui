...
export class CalendarUtils {
  public static compareCalendarDate(date1: OgCalendarDate, date2: OgCalendarDate) {
    return date1.year === date2.year && date1.month === date2.month && date1.date === date2.date;
  }

  public static compareCalendarDate2Moment(date: OgCalendarDate, moment: Moment) {
    return date.year === moment.get('y') && date.month === moment.get('M') && date.date === moment.get('D');
  }

  public static moment2CalendarDate(moment: Moment): OgCalendarDate {
    return { year: moment.get('y'), month: moment.get('M'), date: moment.get('D') };
  }

  public static calendarDate2Moment(date: OgCalendarDate, locale?: string): Moment {
    if (!date) {
      return;
    }
    const result = moment();
